...
class GetCalLog(BaseModel):
    is_cal_file_flg: int = Field(
        isin=[0, 1],
        title="is_cal_file_flg",
        description="0: not exist, 1: exist",
    )
    get_cal_status: Literal["success", "failure"] = Field(
        "succsess",
        title="result",
        description="success or failure",
    )
    get_cal_error_message: StrOrNone = Field(
        default="",
        title="message",
        description="message",
    )
...
class GetPresentationLog(BaseModel):
    is_pre_file_flg: int = Field(
        isin=[0, 1],
        title="is_pre_file_flg",
        description="0: not exist, 1: exist",
    )
    ...
    get_pre_status: Literal["success", "failure"] = Field(
        "succsess",
        title="result",
        description="success or failure",
    )
    get_pre_error_message: StrOrNone = Field(
        default="",
        title="message",
        description="message",
    )
...
class ParentChildLink(pa.DataFrameModel):
    ...
    parent_key: Series[
        str
    ]  # = pa.Column(str, checks=pa.Check.str_contains(':'), regex=True, nullable=True)
    child_key: Series[
        str
    ]  # = pa.Column(str, checks=pa.Check.str_contains(':'), regex=True, nullable=True)
    role: Series[str]
...
class AccountLinkTracer:
    ...
    def __init__(self, tbl: ParentChildLink):
        ...
        self.all_key_list = list(
            set(self.parent_child_tbl.parent_key)
            | set(self.parent_child_tbl.child_key),
        )
    ...
    def get_child_keys(self, parent_key: str, role: str) -> list:
        if (
            len(
                self.parent_child_tbl.query(
                    "parent_key==@parent_key and role == @role",
                ),
            )
            > 0
        ):
            return self.parent_child_tbl.query(
                "parent_key==@parent_key and role == @role",
            ).child_key.to_list()
        return []
    ...
    def get_child_items(
        ...
        tmp_tbl = tmp_tbl.assign(
            child_order_str=now_str + "_" + tmp_tbl.child_order.astype(str),
            layer=str(rec),
        )
        tmp_tbl.child_order = tmp_tbl.child_order.astype(float) * ((0.01) ** rec) + now

        return tmp_tbl.query("parent_key==@parent_key and role == @role")[
            ["child_key", "role", "child_order", "child_order_str", "layer"]
        ].to_dict("records")

    def get_parent_keys(self, child_key: str, role: str) -> list:
        if (
            len(self.parent_child_tbl.query("child_key==@child_key and role == @role"))
            > 0
        ):
            return self.parent_child_tbl.query(
                "child_key==@child_key and role == @role",
            ).parent_key.to_list()
        return []
    ...
    def get_parent_items(self, child_key: str, role: str) -> list:
        if (
            ...
            return self.parent_child_tbl.query(
                "child_key==@child_key and role == @role",
            )[["parent_key", "role"]].to_dict("records")
        return []

    def get_role(self, key: str) -> list:
        return list(
            set(
                self.parent_child_tbl.query("parent_key==@key").role.to_list()
                + self.parent_child_tbl.query("child_key==@key").role.to_list(),
            ),
        )
        ...
        if len(child_key_list) > 0:
            return flatten_list(
                remove_empty_lists(
                    child_key_list
                    + [
                        self.get_child_keys_recursive(child_key, role)
    ...
    def get_child_items_recursive(
        ...
        # child_key_list = self.get_child_keys(parent_key,role)
        child_item_list = self.get_child_items(
            parent_key,
            role,
            now=now,
            now_str=now_str,
        ...
        if len(child_item_list) > 0:
            return flatten_list(
                remove_empty_lists(
                    child_item_list
                    + [
                        self.get_child_items_recursive(
                            child_item["child_key"],
                            role,
                            now=child_item["child_order"],
                            now_str=child_item["child_order_str"],
                            rec=rec + 1,
                        )
    ...
    def get_parent_keys_trace(self, child_key: str, role: str) -> list:
        ...
        if len(parent_key_list) > 0:
            return flatten_list(
                remove_empty_lists(
                    [
                        self.get_parent_keys_trace(parent_key, role)
    ...
    def get_child_order_recursive_list(self, key_list: list, role: str) -> pd.DataFrame:
        ...
        all_keys_df = (
            ...
            .query("child_key in @child_keys")
            .rename(columns={"child_order": "order", "child_order_str": "order_str"})
        )
...
class account_link_tracer:
    ...
    def get_child_order_recursive_list(self, key_list: list, role: str) -> pd.DataFrame:
        ...
        all_keys_df = (
            pd.DataFrame(all_keys)
            .query("child_key in @child_keys")
            .rename(columns={"child_order": "order"})
        )
        return all_keys_df.sort_values("order")
...
class get_presentation_account_list:
    ...
    def __init__(
        ...
        temp_path_str: str,
        doc_type: str = "public",
    ):  # ->(parent_child_link_schima,,dict,dict):
        self.log_dict = {
            #'docID':docid,
        ...
        try:
            with ZipFile(str(zip_file_str)) as zf:
                fn = [
                    ...
                    if ("pre.xml" in item) & (self.doc_type_str in item)
                ]
                if len(fn) > 0:
        ...
        try:
            with ZipFile(str(zip_file_str)) as zf:
                fn = [
                    item
                    for item in zf.namelist()
                    if ("cal.xml" in item) & (self.doc_type_str in item)
                ]
                if len(fn) > 0:
                    zf.extract(fn[0], self.temp_path)
...
class get_label:
    ...
    def parse_lab_file(self):
        tree = ET.parse(
            str(list(self.xml_def_path.glob("*" + self.f_keyword))[0]),
        )  # TODO check iregular file name
        root = tree.getroot()
    ...
    def export_label_tbl(self, label_to_taxonomi_dict: dict) -> pd.DataFrame:
        ...
        label_tbl = AccountLabel(
            label_tbl.assign(
                ...
                key=label_tbl.label_lab.replace(self.label_to_prelabel_dict).replace(
...
def get_presentation_account_list_aud(
    docid: str,
    identifier: str,
    out_path,
) -> (ParentChildLink, OriginalAccountList, dict, dict):
    """locator:
        (role:)
        href:
        label:
    ...
    try:
        ...
        with ZipFile(str(zip_file)) as zf:
            fn = [
                item for item in zf.namelist() if ("pre.xml" in item) & ("aai" in item)
            ]
            if len(fn) > 0:
    ...
    # dict_t['org_taxonomi_list'] = pre_detail_list.query("schima_taxonomi.str.contains(@identifier)").schima_taxonomi.to_list()
        ...
        p_edges_df = ParentChildLink(
            pd.DataFrame(columns=get_columns_df(ParentChildLink)),
        )
        pre_detail_list = OriginalAccountList(
            pd.DataFrame(columns=get_columns_df(OriginalAccountList)),
        )

    return (
        p_edges_df[get_columns_df(ParentChildLink)],
        pre_detail_list[get_columns_df(OriginalAccountList)],
        label_to_taxonomi_dict,
        dict_t,
    )

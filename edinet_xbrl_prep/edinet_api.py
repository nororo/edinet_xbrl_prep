...
class EdinetResponse(BaseModel):
    ...
    seqNumber: int = Field(
        ...,
        title="seq number",
        description="The number YYYY/MM/DD-senCumber, which is given to documents submitted on the same day in the order of submission time, becomes the submission order information.",
    )
    docID: StrOrNone = Field(
        "",
        title="document id",
        description="filename of document (docID.zip)",
    )
    edinetCode: StrOrNone = Field("", title="EDINET code", description="EDINET code")
    secCode: StrOrNone = Field(
        "",
        title="Securities Code",
        description="Securities Code",
    )
    JCN: StrOrNone = Field(
        "",
    ...
    filerName: StrOrNone = Field(
        "",
        title="Name of submitter",
        description="Name of submitter",
    )
    fundCode: StrOrNone = Field("", title="Fund code", description="Fund code")
    ordinanceCode: StrOrNone = Field(
        "",
        title="government ordinance code",
        description="government ordinance code",
    )
    formCode: StrOrNone = Field("", title="form code", description="form code")
    docTypeCode: StrOrNone = Field(
        "",
        title="document type code",
        description="document type code",
    )
    periodStart: StrOrNone = Field(
        "",
        title="start period",
        description="start period(YYYY-MM-DD)",
    )
    periodEnd: StrOrNone = Field(
        "",
        title="end period",
        description="end period(YYYY-MM-DD)",
    )
    submitDateTime: StrOrNone = Field(
        "",
        title="submit date time",
        description="submit date time(YYYY-MM-DD HH:MM:SS)",
    )
    docDescription: StrOrNone = Field(
        "",
        title="document description",
        description="String displayed in the 'Documents submitted' field on the document search results screen of the EDINET browse site.",
    )
    issuerEdinetCode: StrOrNone = Field(
        "",
        title="issuer EDINET code",
        description="Issuer company EDINET code Large holding EDINET code of the issuing company",
    )
    subjectEdinetCode: StrOrNone = Field(
        "",
        title="subject EDINET code",
        description="Target EDINET code for the tender offer.",
    )
    subsidiaryEdinetCode: StrOrNone = Field(
        "",
        title="subsidiary EDINET code",
        description="The EDINET code of the subsidiary is output. If more than one exists (up to 10), the strings are output concatenated by ',' (comma).",
    )
    currentReportReason: StrOrNone = Field(
        "",
        title="current report reason",
        description="The reasons for submitting an extraordinary report and reasons for submitting an extraordinary report are output. If there is more than one, the strings are output as ',' (comma) concatenated strings.",
    )
    parentDocID: StrOrNone = Field(
        "",
        title="parent document management number",
        description="Parent document management number",
    )
    opeDateTime: StrOrNone = Field(
        "",
        title="operation date time",
        description="The date and time when the document was modified by the staff of the Financial Bureau, the document was not disclosed by the staff of the Financial Bureau, and the magnetic disk submission and paper submission were made.",
    )
    withdrawalStatus: StrOrNone = Field(
        "",
        title="withdrawal status",
        description="Withdrawal draft is '1', withdrawn documents are '2', and others are '0'.",
    )
    docInfoEditStatus: StrOrNone = Field(
        "",
        title="document information edit status",
        description="Information that the staff of the Financial Bureau modified the document is '1', the modified document is '2', and others are '0'.",
    )
    disclosureStatus: StrOrNone = Field(
        "",
        title="disclosure status",
        description="Information that the staff of the Financial Bureau started non-disclosure of the document is '1', the document that is not disclosed is '2', the information that the staff of the Financial Bureau released the non-disclosure of the document is '3', and others are '0'.",
    )
    xbrlFlag: StrOrNone = Field(
        "",
        title="XBRL flag",
        description="If the document has XBRL, it is '1', otherwise 0.",
    )
    pdfFlag: StrOrNone = Field(
        "",
        title="PDF flag",
        description="If the document has PDF, it is '1', otherwise 0.",
    )
    attachDocFlag: StrOrNone = Field(
        "",
        title="attach document flag",
        description="If the document has an alternative document/attachment, it is '1', otherwise 0.",
    )
    englishDocFlag: StrOrNone = Field(
        "",
        title="english document flag",
        description="If the document has an English file, it is '1'.",
    )
    csvFlag: StrOrNone = Field(
        "",
        title="csv flag",
        description="If the document has a csv, it is '1'.",
    )
    legalStatus: StrOrNone = Field(
        "",
        title="legal status",
...
class EdinetMetadataInputV2(BaseModel):
    ...
    type_api_param: int = Field(
        isin=[1, 2],
        title="type",
        description="1: Retrieves metadata only. 2: Retrieve the list of submitted documents and metadata.",
        default=2,
    )
    ...
    def export(self):
        return {
            "date": self.date_api_param,
            "type": self.type_api_param,
...
def get_edinet_metadata(params: EdinetMetadataInputV2) -> RequestResponse:
    ...
    result_temp = {
        "date_res": params.date_api_param,
        "status": "success",
        "data": [],
        "message": None,
    }
    ...
    if res.status_code == 200:
        ...
        try:
            ...
            for res_day in res_parsed["results"]:
                ...
                res_list.append(
                    {key: res_day[key] for key in get_columns(EdinetResponse)},
                )
            result_temp["data"] = res_list

        except json.JSONDecodeError as e:
            result_temp["status"] = "failure"
            result_temp["message"] = f"JSON Decoding Error: {e!s}"
        except Exception as e:
            print(e)
            result_temp["status"] = "failure"
            result_temp["message"] = f"Error: {e!s}"
    else:
        result_temp["status"] = "failure"
...
class edinet_response_metadata:
    ...
    def __init__(
        self,
        filename=None,
        tse_sector_url: str = None,
        tmp_path_str: str = None,
    ):
        if filename:
    ...
    def get_metadata_pandas_df(self) -> pd.DataFrame:
        return pd.concat(
            [pd.DataFrame(data.model_dump()["data"]) for data in self.data],
        ).reset_index()
        ...
        if self.tse_sector_url:
            ...
            business_class = pd.read_excel(
                ...
                columns={
                    ...
                    "33業種区分": "sector_label_33",
                    "17業種コード": "sector_code_17",
                    "17業種区分": "sector_label_17",
                },
            )[
                [
                    "date",
...
def request_term(
    ...
    res_results = []
    for itr in tqdm(range((end_date - start_date).days + 1)):
...
class EdinetDocInputV2(BaseModel):
    type_api_param: int = Field(
        isin=[1, 2, 5],
        title="type",
        description="1: xbrl, 2: pdf, 5:csv",
        default=1,
    )
    api_key: str = Field(..., title="Subscription-Key", description="API Key")

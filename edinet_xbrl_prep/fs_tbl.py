...
from .link_base_file_analyzer import AccountLinkTracer
from .utils import *
...
class LinkbaseFile:
    ...
    def read_linkbase_file(self):
        ...
        self.label_tbl_jp = self.get_label_obj_jp.export_label_tbl(
        ...
        self.label_tbl_eng = self.get_label_obj_eng.export_label_tbl(
            label_to_taxonomi_dict=self.get_presentation_account_list_obj.export_label_to_taxonomi_dict(),
        )
    ...
    def make_account_label(
        ...
        self.account_link_tracer_obj = AccountLinkTracer(self.parent_child_df)
        ...
        if len(role_list) > 0:
            ...
            for role_t in role_list:
                role_list_f = role_list_f + [
                    role_key for role_key in role_list_all if role_t in role_key
                ]
        else:
        ...
        for role_text in role_list_f:
            ...
            account_tbl_of_the_role = account_tbl.query(
                "role.str.contains(@role_suffix)",
            ).drop_duplicates()
            account_tbl_of_the_role = account_tbl_of_the_role.assign(
                parent_key_list=account_tbl_of_the_role.key.apply(
                    lambda x: self.account_link_tracer_obj.get_parent_keys(
                        child_key=x,
                        role=role_text,
                    ),
                ),
            ...
            account_tbl_of_the_role = pd.merge(
                ...
                self.account_link_tracer_obj.get_child_order_recursive_list(
                    ...
                    role=role_text,
                )[["order", "order_str", "child_key", "layer"]],
                left_on="key",
                right_on="child_key",
                how="left",
            )
            # account_tbl_of_the_role.order.fillna(1,inplace=True)
            # account_tbl_of_the_role.parent_key_list = account_tbl_of_the_role.parent_key_list.fillna([]) # change
            account_tbl_of_the_role.order = account_tbl_of_the_role.order.fillna(
                1,
            )  # change
            account_tbl_of_the_role.order_str = (
                account_tbl_of_the_role.order_str.fillna("0")
            )  # change
    ...
    def make_account_label_org(self):
        df = (
            self.label_tbl_jp.query("role == 'label'")
            .set_index("key")
            .rename(columns={"text": "label_jp"})
        )
        df.join(
            [
                self.label_tbl_jp.query("role == 'verboseLabel'")
                .set_index("key")[["text"]]
                .rename(columns={"text": "label_jp_long"}),
                self.label_tbl_eng.query("role == 'label'")
                .set_index("key")[["text"]]
                .rename(columns={"text": "label_en"}),
                self.label_tbl_eng.query("role == 'verboseLabel'")
                .set_index("key")[["text"]]
